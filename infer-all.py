...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="River inference — shape + centerline + width")
...
def main() -> None:
    ...
    for tif_path in input_files:
        ...
        logger.info("--- shape ---")
        infer_shape(mask, meta, rgb, build_output_dir(base_out, tif_path, "shape"), stem)

        logger.info("--- centerline ---")
        infer_centerline(mask, meta, rgb, build_output_dir(base_out, tif_path, "centerline"), stem, skeleton)

        logger.info("--- width ---")
        infer_width(mask, skeleton, meta, rgb, build_output_dir(base_out, tif_path, "width"), stem, sword_dir)

        print(f"Done  ->  {base_out / tif_path.parent.name}/")
